...
sentences = {}
with open("./stanfordSentimentTreebank/datasetSentences.txt", "rb") as f:
...
dev = {}
sents = []
with open("./stanfordSentimentTreebank/datasetSplit.txt", "rb") as f:
	rd=csv.reader(f,delimiter=',')
	count = 0
	for line in rd:
		if count == 0:
...
string = " ".join(sents)
with open("./stanfordSentimentTreebank/dictionary.txt", "rb") as f:
	rd=csv.reader(f,delimiter='|')
	for line in rd:
...
labels={}
with open("./stanfordSentimentTreebank/sentiment_labels.txt", "rb") as f:
	rd=csv.reader(f,delimiter='|')
	count=0
	for line in rd:
		if count==0:
...
with open("sst2_train_phrases.csv", "wb") as f:
	wr=csv.writer(f,delimiter='	')
	for key in train:
		x=train[key]
		if x<=0.4:
...
with open("sst2_train_sentence.csv", "wb") as f:
	wr=csv.writer(f,delimiter='	')
	for key in train_sent:
		x=train_sent[key]
		if x<=0.4:
...
with open("sst2_test.csv","wb") as f:
	wr=csv.writer(f,delimiter='	')
	for key in test:
		x=test[key]
		if x<=0.4:
...
with open("sst2_dev.csv", "wb") as f:
	wr=csv.writer(f,delimiter='	')
	for key in dev:
		x=dev[key]
		if x<=0.4:

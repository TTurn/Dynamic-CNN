...
class DCNN():
    ...
    def conv_layer(self, x, w, b):
        ...
        with tf.name_scope("per_dim_conv"):
            for i in range(len(input_unstack)):
                ...
                conv = tf.nn.relu(conv + b_unstack[i])#[batch_size, k1+ws2-1, num_filters[1]]
                convs.append(conv)
    ...
    def run(self, sent, W1, b1, top_k, Wh, bh, Wo, dropout_keep_prob):
        conv1 = self.conv_layer(sent, W1, b1)
        k = int(int(sent.shape[1])/2)
        print(k)
        conv1 = self.fold_k_max_pooling(conv1, top_k)
        #conv2 = self.conv_layer(conv1, W2, b2)
        #fold = self.fold_k_max_pooling(conv2, top_k)
        fold_flatten = tf.reshape(conv1, [-1, int(top_k*self.embed_size*self.num_filters[0]/2)])

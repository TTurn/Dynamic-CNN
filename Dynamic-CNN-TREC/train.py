...
# Load data
print("Loading data...")
x_, y_, vocabulary, test_size, embedding = word_embedding_train.load_data()
#pretrain word embedding: https://blog.csdn.net/lxg0807/article/details/72518962
...
with tf.name_scope("embedding_layer"):
    W = tf.Variable(tf.constant(0.0, shape=[len(vocabulary)+1, embed_dim]), trainable=False, name="W")
    embedding_placeholder = tf.placeholder(tf.float32, [len(vocabulary)+1, embed_dim])
...
Wh = init_weights([int(top_k*embed_dim*num_filters[0]/2), num_hidden], "Wh")
...
out = dcnn.run(input_x, W1, b1, top_k, Wh, bh, Wo, dropout_keep_prob)

with tf.name_scope("cost"):
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=out, labels=y))+tf.contrib.layers.l2_regularizer(0.0005)(W1)+tf.contrib.layers.l2_regularizer(0.005)(Wh)+tf.contrib.layers.l2_regularizer(0.005)(Wo)
...
with tf.Session() as sess:
    ...
    batches = word_embedding_train.batch_iter(zip(x_train, y_train), batch_size, n_epochs)
